...
type DeleteMatchTicketResponse struct {
	...
	Error406 *match2clientmodels.ResponseError
	Error500 *match2clientmodels.ResponseError
}

func (m *DeleteMatchTicketResponse) Unpack() *match2clientmodels.ApiError {
	if !m.IsSuccess {
		var errCode int
		...
		case 406:
			e, err := m.Error406.TranslateToApiError()
			if err != nil {
				_ = fmt.Errorf("failed to translate error. %v", err)
			}
