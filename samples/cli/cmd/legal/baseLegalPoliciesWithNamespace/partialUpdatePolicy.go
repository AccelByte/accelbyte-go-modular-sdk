...
var PartialUpdatePolicyCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		input := &base_legal_policies_with_namespace.PartialUpdatePolicyParams{
			Body:         body,
			BasePolicyID: basePolicyId,
			Namespace:    namespace,
		}
		ok, errOK := baseLegalPoliciesWithNamespaceService.PartialUpdatePolicyShort(input)
		if errOK != nil {
			logrus.Error(errOK)
...
func init() {
	PartialUpdatePolicyCmd.Flags().String("body", "", "Body")
	PartialUpdatePolicyCmd.Flags().String("basePolicyId", "", "Base policy id")
	_ = PartialUpdatePolicyCmd.MarkFlagRequired("basePolicyId")
	PartialUpdatePolicyCmd.Flags().String("namespace", "", "Namespace")
	_ = PartialUpdatePolicyCmd.MarkFlagRequired("namespace")
}
