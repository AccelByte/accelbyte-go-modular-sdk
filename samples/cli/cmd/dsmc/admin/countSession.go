...
var CountSessionCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		input := &admin.CountSessionParams{
			Namespace: namespace,
			Region:    &region,
		}
		ok, errOK := adminService.CountSessionShort(input)
		if errOK != nil {
			logrus.Error(errOK)
...
func init() {
	CountSessionCmd.Flags().String("namespace", "", "Namespace")
	_ = CountSessionCmd.MarkFlagRequired("namespace")
	CountSessionCmd.Flags().String("region", "", "Region")
}
