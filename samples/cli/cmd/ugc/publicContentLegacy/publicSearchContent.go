...
var PublicSearchContentCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		input := &public_content_legacy.PublicSearchContentParams{
			Namespace:  namespace,
			Creator:    &creator,
			Ishidden:   &ishidden,
		...
		ok, errOK := publicContentLegacyService.PublicSearchContentShort(input)
		if errOK != nil {
			logrus.Error(errOK)
...
func init() {
	...
	PublicSearchContentCmd.Flags().String("ishidden", "", "Ishidden")
