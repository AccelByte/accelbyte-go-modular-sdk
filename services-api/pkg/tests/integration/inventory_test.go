...
var (
	adminInventoriesService = &inventory.AdminInventoriesService{
		Client:          inventory.NewInventoryClient(configRepo),
...
func TestIntegrationInventory(t *testing.T) {
	...
	if createInventoryConfig != nil {
		t.Logf("Created inventory config with code: %v, id: %v", *createInventoryConfig.Data.Code, *createInventoryConfig.Data.ID)
	}

	defer func() {
		_ = deleteInventoryConfiguration(t, *createInventoryConfig.Data.ID)
	}()
	...
	inputCreateInventory := &admin_inventories.AdminCreateInventoryParams{
		Body: &inventoryclientmodels.ApimodelsCreateInventoryReq{
			InventoryConfigurationCode: createInventoryConfig.Data.Code,
			UserID:                     &userID,
		},
		Namespace: integration.NamespaceTest,
	}
	...
	inventoryID := *createInventoryOk.Data.ID
	t.Logf("Created inventory with id: %v", *createInventoryOk.Data.ID)
	...
	t.Logf("Get inventory with id: %v", *getInventory.Data.InventoryConfigurationID)
	...
	assert.NotEqual(t, *updateInventory.Data.MaxSlots, initialMaxSlots)

	messageInventory := "deleting " + inventoryConfigurationCode
	t.Logf("Update inventory with max slot: %v", *createInventoryOk.Data.ID)
