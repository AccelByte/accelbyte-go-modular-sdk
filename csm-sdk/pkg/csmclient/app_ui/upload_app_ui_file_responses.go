...
type UploadAppUIFileResponse struct {
	csmclientmodels.ApiResponse
	Data *csmclientmodels.ApimodelUploadFileResponse
	...
	Error502 *csmclientmodels.ResponseErrorResponse
}

func (m *UploadAppUIFileResponse) Unpack() (*csmclientmodels.ApimodelUploadFileResponse, *csmclientmodels.ApiError) {
	if !m.IsSuccess {
		var errCode int
		...
		case 502:
			e, err := m.Error502.TranslateToApiError()
			if err != nil {
				_ = fmt.Errorf("failed to translate error. %v", err)
			}
